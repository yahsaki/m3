...
const libraryPath = 'C:\\Users\\admin\\Music\\_m3_test'

const api = {
  generatePlaylist: (scanMusicFolderRes) => {
    const playlist = {
      basePath: scanMusicFolderRes.basePath,
      tracks: [],
    }
    for (let i = 0; i < scanMusicFolderRes.labels.length; i++) {
      const label = scanMusicFolderRes.labels[i]
  ...
  scanMusicFolder: () => {
    // this assumes bandcamp style folder structure at the moment
    const result = {
      basePath: libraryPath,
      labels: [],
    }
    const labelNames = fs.readdirSync(libraryPath)
    for (let i = 0; i < labelNames.length; i++) {
      ...
      const labelPath = path.join(libraryPath, labelName)
