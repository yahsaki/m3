    ...
    for (let i = 0; i < source.data.length; i++) {
      ...
      for (let j = 0; j < label.releases.length; j++) {
        ...
        for (let k = 0; k < release.tracks.length; k++) {
          ...
          playlistTrack.coverPath = playlistTrack.coverPathPart ? path.join(playlist.basePath, playlistTrack.coverPathPart) : ""
          playlistTrack.logoPath = playlistTrack.logoPathPart ? path.join(playlist.basePath, playlistTrack.logoPathPart) : ""
          playlistTrack.bannerPath = playlistTrack.bannerPathPart ? path.join(playlist.basePath, playlistTrack.bannerPathPart) : ""
